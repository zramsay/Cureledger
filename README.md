...
This prototype uses Scaffold-Eth-2 token vendor as its base due to well-maintained web3 infrastructure. All we have to do is change our RPC endpoints to move from Hardhat to Avalanche testnet.
...
configured to mint 1B tokens at 35000 per Eth.
Vendor Contract - this contract is loaded with tokens to distribute the DSCI in the app.


## Vendor.sol - 

needs to be loaded with tokens after minting, so the mint process needs to take this into account.


## Community - Questions Page  ==> Claim DSCI Page

### We can use this on TestNet and let users redeem for Mainnet tokens.

I currently have this set to distribute 10 tokens for free after answering questions about one's DeSci B2B company. This is a baby-step airdrop.

I also have a commercial sale NFT created in order for the users to buy our software with DSCI, but that is a next month issue when we have the commercial license terms ready.


## The Token-Vendor page. 

### We may not want to publish this page yet. We may want to use token warrants for the short term rather than a sale.

This is where we sell tokens in exchange for currency. It needs another vendor contract for selling rather than giveaways. I have not finished this because I think our point of sale for this may be Stripe Crypto or Shopify due to compliance, not a token swap.


## Inspect Contracts, Events & Block explorer

These utilities built into scaffold-eth-2 and help us test our platform & transfer tokens as needed.


# TODO

## Decide on wallet and workflow for token vendor; update contracts

## Change RPC endpoints to Avalanche 

## Get some test Avax & Deploy




# Leaving Scaffold-Eth ReadMe for now
